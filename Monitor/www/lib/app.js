...
angular.module('SeeTheWonder', ['AngularForce', 'AngularForceObjectFactory', 'Contact', 'Account', 'Lead', 'Wonder__c']);
...
angular.module('Account', []).factory('Account', function (AngularForceObjectFactory) {
    ...
    var objDesc = {
        type: 'Account',
        fields: ['Id', 'Name', 'Type', 'BillingCity'],
        where: '',
        orderBy: 'Name ASC',
        limit: 20
    };
...
angular.module('SeeTheWonder').controller('LeadDetailCtrl', ['$scope', '$location','Lead', 'AngularForce','SFConfig','$rootScope',
    function($scope, $location,Lead,AngularForce,SFConfig,$rootScope) {
        ...
        $scope.doSave = function() {
            ...
            Lead.save($scope.newLead,
                function(data) {
                    //alert('success!');
                    $location.path("/leads");
                    $scope.apply(function() { $location.path("/leads"); });
                },
                function(data) {
                logToConsole('QUERYERROR');
